...
public class ap1 {
	...
	public int scoresAverage(int[] scores) {
		...
		int firstAvg = average(scores, 0, mid - 1);
		int secondAvg = average(scores, mid, scores.length - 1);
	...
	public boolean dividesSelf(int n) {
		...
		return (count == divCount) ? true : false;
	}
	...
	public int[] copyEvens(int[] nums, int count) {
		...
		for (int i = 0; i < nums.length; i++) {
			if (nums[i] % 2 == 0 && counter < newArr.length) {
	...
	public int[] copyEndy(int[] nums, int count) {
		...
		int counter = 0;
		for (int i = 0; i < nums.length; i++) {
			if ((nums[i] >= 0 && nums[i] <= 10 || nums[i] >= 90 && nums[i] <= 100) && counter < newArr.length) {
				newArr[counter++] = nums[i];
			}
		}
		return newArr;
	...
	public String[] wordsWithout(String[] words, String target) {
		...
		for (int i = 0; i < words.length; i++) {
			if (words[i] != target) {
				count--;
				newArr[newArr.length - 1 - count] = words[i];
			}
		}
		return newArr;
	}
	...
	public int sumHeights(int[] heights, int start, int end) {
		int sum = 0;
		for (int i = start; i < end; i++) {
			sum = sum + ((heights[i] - heights[i + 1]) < 0 ? (heights[i] - heights[i + 1]) * (-1)
